...
function output_signlist(data){  //清空元素先
	...
	for(var i=0;i<data.length;i++){//依次输出返回数据到表格中
		var appenstr='<tr><td>'+data[i].name+'</td><td>'+data[i].tel+'</td><td>'+data[i].address+'</td><td>'+data[i].department+'</td><td><button class="btn btn-xs btn-alt m-r-5" data-appid="'+data[i].appid+'" onclick="Sign_action_exit(this)" style="padding-top: -10px;">签退</button></td></tr>';
...
function Get_sign_list(aid,search){
 ...
 function Sign_action_exit(obj){
	layer.confirm('<span style="color:#000000">确认要签退吗？</span>',function(index){
		appid=$(obj).data('appid');
		var aid=GetQueryString('aid');
		$.get('sign_exit.php',{aid:aid,action:"confirm",appid:appid});
