...
if (!empty($_POST['trade_no'])&& trim($_POST['trade_no'])!=""){
	$f2fpay = new F2fpay();
	
	$out_trade_no = trim($_POST['trade_no']);
	$refund_amount = trim($_POST['refund_amount']);
	$out_request_no = trim($_POST['out_request_no']);
	
	$response = $f2fpay->refund($out_trade_no,$refund_amount,$out_request_no);
	print_r($response);
	return ;
}
