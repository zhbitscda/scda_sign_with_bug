...
if(isset($_SESSION['uid'])){
	...
	if($_GET['action']=='cancel'){
		$db->update_data_basic('apply', array('status'), array(0), 'appid', '=', $_GET['appid']);
	}
	switch($_GET['type']){
		case 1://未到
        ...
        $r=$db->all_query_stmt('SELECT appid,name,tel,address,department,apply.personid FROM apply,person where apply.personid=person.personid AND apply.status=1 AND apply.actid=?', array($_GET['aid']));	
		break;
		default:
		//$db_header=$db->db_header;
		//$number_of_all=$db->db_query_n1_count('apply', 'actid', $_GET['actid'],'=');
